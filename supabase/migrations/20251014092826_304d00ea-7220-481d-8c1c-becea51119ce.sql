-- Clean migration: Drop everything and recreate
...
BEGIN
    ...
    FOR r IN (SELECT schemaname, tablename, policyname 
              ...
              AND tablename IN ('departments', 'profiles', 'user_roles', 'services', 'service_requests', 'payments', 'feedback', 'audit_logs'))
    LOOP
...
CREATE POLICY "Only super_admin can manage departments" 
  ON public.departments FOR ALL USING (public.has_role(auth.uid(), 'super_admin'));
...
CREATE POLICY "Department admins can manage their services" 
...
CREATE POLICY "Citizens can view their own requests" 
  ON public.service_requests FOR SELECT USING (auth.uid() = citizen_id);
CREATE POLICY "Citizens can create requests" 
  ON public.service_requests FOR INSERT WITH CHECK (auth.uid() = citizen_id);
...
CREATE POLICY "Users can view their own feedback" 
  ON public.feedback FOR SELECT USING (auth.uid() = citizen_id);
CREATE POLICY "Users can create feedback" 
  ON public.feedback FOR INSERT WITH CHECK (auth.uid() = citizen_id);
...
INSERT INTO public.services (department_id, name, description, fee, processing_time)
SELECT d.id, v.name, v.description, v.fee, v.processing_time
FROM public.departments d
CROSS JOIN (
  VALUES 
    ('Revenue Department', 'Land Record Certificate', 'Get certified copy of land records', 50.00, '7 days'),
    ('Transport Department', 'Driving License Renewal', 'Renew your expired driving license', 200.00, '15 days'),
    ('Municipal Corporation', 'Birth Certificate', 'Obtain official birth certificate', 30.00, '5 days')
) AS v(dept_name, name, description, fee, processing_time)
WHERE d.name = v.dept_name;